...
function validator(){
	...
	/* 
	 * Function validate - return either true or false
	 * Param input (string) (required) - input string to validate
	 * Param against (string) (optional) - test to match against - required for some functions only - see the html attached for usage examples. 
	*/

	validator.prototype.validate = function(input,against) {
		input = input.trim(); // always trim, this can easily be removed if requred.
		var rules = [];
		...
		rules['postcode'] = /^[a-zA-Z]{1,2}\d{1,2}\s?\d[a-zA-Z]{1,2}$/; // UK format only
	 ...
	 * Param input (string) - the string to auto-correct
	 * Param to (string) - the rule to autocorrect to - see the example html for full list of auto-correct rules
...
var _v = new validator(); // if you don't want to modify the string prototype, you can call directly using _v.[function name](input,type);
